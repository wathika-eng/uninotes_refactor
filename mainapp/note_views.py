...
def course_details(request: HttpRequest, course_id: int):
    ...
    units_with_notes = course.units.annotate(note_count=Count("notes")).filter(note_count__gt=0)

    units_by_year = defaultdict(list)
    for unit in units_with_notes:
        units_by_year[unit.year_of_study].append(unit)

    return render(request, "mainapp/course_details.html", {
        "course": course,
    ...
    if request.method == "POST":
        try:
            ...
            if not uploaded_files:
                return JsonResponse({"success": False, "message": "At least one file must be uploaded."})
            ...
            for uploaded_file in uploaded_files:
                ...
                if uploaded_file.size > 10 * 1024 * 1024:  # Example: max file size 10MB
                    return JsonResponse({"success": False, "message": "File size exceeds limit (10MB)."})
